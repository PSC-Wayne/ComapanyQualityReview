...
const stageOrder=['queued','collecting_official_evidence','evidence_bundle_complete'];
...
function renderResult(result){resultPanel.classList.remove('hidden');const rows=result.source_coverage||[];document.querySelector('#coverage').innerHTML=rows.map(row=>`<div class="metric"><span class="muted">${escapeHtml(row.family)}</span><strong class="${row.available===row.required?'coverage-ok':'coverage-gap'}">${row.available} / ${row.required}</strong><span class="muted">${row.missing_reasons?.length||0} 個缺口</span></div>`).join('');document.querySelector('#raw-result').textContent=JSON.stringify(result,null,2)}
