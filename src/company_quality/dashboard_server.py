...
<section id="result-panel" class="panel result hidden"><h2>研究結果</h2><div id="report-summary"></div><section data-section-id="financial_deterioration"><h2 style="margin-top:22px">財報惡化</h2><div id="downside-financial"></div></section><section data-section-id="unexplained_financial_anomalies"><h2 style="margin-top:22px">無法解釋財報異常</h2><div id="downside-anomalies"></div></section><section data-section-id="recent_negative_news"><h2 style="margin-top:22px">近期負面新聞</h2><div class="muted">affected account / cash flow · realised / hypothetical · duplicate cluster</div><div id="downside-news"></div></section><section data-section-id="three_year_kam" data-contract="kam_judgement risk_mechanism"><h2 style="margin-top:22px">三年KAM（KAM問題）</h2><div class="muted">KAM存在本身不等於問題；查核意見、繼續經營、強調事項與會計師異動分開顯示。</div><div id="downside-kam"></div></section><h2 style="margin-top:22px">上漲潛力</h2><div id="upside" class="status-grid"></div><h2 style="margin-top:22px">估值情境（research_only）</h2><div id="valuation" class="status-grid"></div><h2 style="margin-top:22px">報酬機率</h2><div id="probabilities" class="status-grid"></div><h2 style="margin-top:22px">Evidence Coverage</h2><div id="coverage" class="coverage-grid"></div><h2 style="margin-top:22px">本機財報庫</h2><div id="filing-store" class="coverage-grid"></div><h2 style="margin-top:22px">官方引用證據</h2><div id="evidence-citations"></div><div id="limitations" class="muted" style="margin-top:14px"></div><details style="margin-top:14px"><summary>查看原始結果 JSON</summary><pre id="raw-result"></pre></details></section>
</main><script>
...
function downsideItem(item){const list=value=>(value||[]).map(text=>`<li>${escapeHtml(text)}</li>`).join('');const confidence=item.confidence==null?'Unavailable':escapeHtml(item.confidence);return `<article class="metric"><span class="muted">severity ${escapeHtml(item.severity)} · confidence ${confidence}</span><strong>${escapeHtml(item.summary)}</strong><h3>evidence</h3><ul>${list(item.evidence)}</ul><h3>counterevidence</h3><ul>${list(item.counterevidence)}</ul><h3>monitoring</h3><ul>${list(item.monitoring)}</ul><h3>invalidation</h3><ul>${list(item.invalidation)}</ul></article>`}
function downsideSection(section){const gaps=(section.gaps||[]).length?` · gaps ${escapeHtml(section.gaps.join('；'))}`:'';return `<div class="muted">status ${escapeHtml(section.status)}${gaps}</div><div class="coverage-grid">${(section.items||[]).map(downsideItem).join('')}</div>`}
...
function renderResult(result){resultPanel.classList.remove('hidden');const report=result.research_report||null;const sections=report&&Array.isArray(report.downside_sections)?report.downside_sections:[];const expected=['financial_deterioration','unexplained_financial_anomalies','recent_negative_news','three_year_kam'];const valid=sections.length===4&&expected.every((id,index)=>sections[index].section_id===id&&sections[index].generation_id===report.generation_id);document.querySelector('#report-summary').innerHTML=valid?`<div class="metric"><span class="muted">publication status ${escapeHtml(report.status)} · generation ${escapeHtml(report.generation_id)}</span><strong>${escapeHtml(report.downside.headline)}</strong></div><div class="status-grid">${caseCard('上漲潛力',report.upside)}</div>`:'<div class="error">舊bundle不相容；不讀取legacy fallback。請重新建立分析。</div>';const targets=['#downside-financial','#downside-anomalies','#downside-news','#downside-kam'];targets.forEach((target,index)=>document.querySelector(target).innerHTML=valid?downsideSection(sections[index]):'');document.querySelector('#probabilities').innerHTML=valid?probabilityCard('12個月絕對正報酬',report.upside.positive_return_probability)+probabilityCard('12個月跑贏官方指數',report.upside.benchmark_outperform_probability)+probabilityCard('12個月內最大跌幅',report.downside.twelve_month_drawdown_probability):'';const rows=valid?report.source_coverage:[];document.querySelector('#coverage').innerHTML=rows.map(row=>`<div class="metric"><span class="muted">${escapeHtml(row.family)}</span><strong class="${row.available===row.required?'coverage-ok':'coverage-gap'}">${row.available} / ${row.required}</strong><span class="muted">${row.missing_reasons?.length||0} 個缺口</span></div>`).join('');const cache=result.filing_store_stats||{hits:0,misses:0,saved:0,corruptions:0};document.querySelector('#filing-store').innerHTML=[['Local hits',cache.hits],['Online misses',cache.misses],['Saved PDFs',cache.saved],['Corruptions',cache.corruptions]].map(item=>`<div class="metric"><span class="muted">${item[0]}</span><strong>${item[1]}</strong></div>`).join('');document.querySelector('#evidence-citations').innerHTML=(valid?report.citations:[]).map(citationCard).join('');document.querySelector('#limitations').textContent=valid?report.limitations.join('；'):'';document.querySelector('#raw-result').textContent=JSON.stringify(result,null,2)}
function renderResult(result){
  resultPanel.classList.remove('hidden');
  const report=result.research_report||null;
  const sections=report&&Array.isArray(report.downside_sections)?report.downside_sections:[];
  const expected=['financial_deterioration','unexplained_financial_anomalies','recent_negative_news','three_year_kam'];
  const valid=result.generation_id===currentGeneration&&report&&report.generation_id===currentGeneration&&report.valuation&&report.valuation.generation_id===currentGeneration&&sections.length===4&&expected.every((id,index)=>sections[index].section_id===id&&sections[index].generation_id===currentGeneration);
  document.querySelector('#report-summary').innerHTML=valid?`<div class="metric"><span class="muted">publication status ${escapeHtml(report.status)} · generation ${escapeHtml(report.generation_id)}</span><strong>${escapeHtml(report.downside.headline)}</strong></div>`:'<div class="error">舊bundle不相容或generation不一致；不讀取legacy fallback。請重新建立分析。</div>';
  const targets=['#downside-financial','#downside-anomalies','#downside-news','#downside-kam'];
  targets.forEach((target,index)=>document.querySelector(target).innerHTML=valid?downsideSection(sections[index]):'');
  document.querySelector('#upside').innerHTML=valid?caseCard('上漲潛力',report.upside):'';
  document.querySelector('#valuation').innerHTML=valid?caseCard('估值情境',report.valuation):'';
  document.querySelector('#probabilities').innerHTML=valid?probabilityCard('12個月絕對正報酬',report.upside.positive_return_probability)+probabilityCard('12個月跑贏官方指數',report.upside.benchmark_outperform_probability)+probabilityCard('12個月內最大跌幅',report.downside.twelve_month_drawdown_probability):'';
  const rows=valid?report.source_coverage:[];
  document.querySelector('#coverage').innerHTML=rows.map(row=>`<div class="metric"><span class="muted">${escapeHtml(row.family)}</span><strong class="${row.available===row.required?'coverage-ok':'coverage-gap'}">${row.available} / ${row.required}</strong><span class="muted">${row.missing_reasons?.length||0} 個缺口</span></div>`).join('');
  const cache=result.filing_store_stats||{hits:0,misses:0,saved:0,corruptions:0};
  document.querySelector('#filing-store').innerHTML=[['Local hits',cache.hits],['Online misses',cache.misses],['Saved PDFs',cache.saved],['Corruptions',cache.corruptions]].map(item=>`<div class="metric"><span class="muted">${item[0]}</span><strong>${item[1]}</strong></div>`).join('');
  document.querySelector('#evidence-citations').innerHTML=(valid?report.citations:[]).map(citationCard).join('');
  document.querySelector('#limitations').textContent=valid?report.limitations.join('；'):'';
  document.querySelector('#raw-result').textContent=JSON.stringify(result,null,2);
}
