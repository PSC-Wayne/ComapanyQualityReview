# T27 — Wayne-only Override Governance Slice

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 為published ratings建立allowlist、期限、independent review與immutable audit trail。
...
- Branch: `wo/T27-override-governance`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T27-override-governance`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T27 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/governance/override/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/governance/override/
...
### Bounded output
- Contract: `OverrideAuditRecord.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/governance/override/contracts/OverrideAuditRecord.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.override_audit`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `override_id:string[1..128], action:oneof[annotation,block,bomb,stricter_floor], pre_output_hash:sha256, post_output_hash:sha256, reason:string[1..1024], evidence_ids:list<string[0..4096]>[1..64], affected_fields:list<string[1..256]>[1..64], effective_at:rfc3339_timezone_aware, expires_at:rfc3339_timezone_aware, max_duration_days:literal[90], expiry_triggers:list<oneof[new_financial_report,new_material_information,new_governed_source_generation,calendar_expiry]>[1..4], recompute_generation_id:uuid, approved_by:literal[Wayne], reviewer_ack:string[1..256], override_coverage:decimal[0,1], rating_disposition:literal[NO_RATING_NOT_APPLICABLE]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
### Explicit non-goals
- Implement override governance only; do not change base model outputs or approve an override.

### Ticket-specific failure disposition
- Override duration is at most 90 calendar days. A new financial report, material information item, governed-source generation or calendar expiry invalidates the override, forces a new immutable generation, deterministic base-output recomputation and fresh independent re-review before any replacement override.
- Action outside allowlist, expired evidence or missing independent ACK is rejected; repeated overrides/model drift triggers governance review.
...
- Focused — **MANDATORY**: `python -m pytest -q tests/governance/override`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/governance/override` plus every previously integrated black-box seam test selected by PM manifest.
...
- PM dispatch manifest is atomic and immutable: `assignment_id`, `active_binding_generation`, `eligibility_generation`, `ticket_id=T27`, `ticket_generation=R9`, `parent_sha`, `branch`, `worktree`, `owned_paths`, `forbidden_paths`, `spec_sha=36edd6b2a1b04c6282a5c30c4b4c5d89ac2535c344d6496a0d8bd54fd2009161`, `decision_map_sha=cc34f1b5f93a28b967e58be2b45f25aca6f700eba72f443dcbb3f8b1ba318b54`, `delivery_plan_sha=bd2b949ab575b01c2553269dd99d67aa385c241c924f35b43cdc1f568bd7c3e0`, `ticket_set_digest`, `lease_expires_at`, `review_deadline_at`. Missing field => no dispatch.
...
- [ ] allow/deny matrix.
- [ ] 保存pre/post/reason/evidence/fields/times/actors.
- [ ] stale/duplicate/expired拒絕.
- [ ] repeated override/model drift觸發review.
- [ ] 無Wayne+independent PASS不生效.
