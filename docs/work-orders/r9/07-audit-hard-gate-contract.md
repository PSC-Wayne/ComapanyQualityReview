# T07 — Audit意見、KAM與Hard-Gate Contract

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 將opinion/going concern/KAM/emphasis/auditor change/integrity events轉成序列化、單調的shared hard gate。
...
- Branch: `wo/T07-audit-hard-gate-contract`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T07-audit-hard-gate-contract`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T07 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/audit/gates/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/contracts/audit_gate.py
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/audit/gates/
...
### Bounded output
- Contract: `AuditGateDecision.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/audit/gates/contracts/AuditGateDecision.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.audit_gate`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `gate_state:oneof[clear,cap,no_rating,blocked], opinion_type:null|oneof[unmodified,qualified,adverse,disclaimer], going_concern:null|boolean, restatement:null|boolean, confirmed_fraud:null|boolean, cap_value:null|int[0,100], floor_value:null|int[1,5], reasons:list<string[1..512]>[1..32], evidence_ids:list<string[0..4096]>[1..64], available_at:rfc3339_timezone_aware, coverage:decimal[0,1], no_rating_reason:null|oneof[formal_disclaimer,formal_adverse,missing_mandatory_evidence,authority_conflict]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.

### Coverage and rating disposition
- Output schema carries an explicit coverage value in `[0,1]`. This ticket does not issue a rating; `rating_disposition=NO_RATING_NOT_APPLICABLE`; missing mandatory evidence follows the ticket-specific blocked/unknown rule, never an implicit score.
...
- Formal disclaimer/adverse/going-concern applies no-rating/cap/floor monotonically; missing evidence is blocked/unknown, not a formal opinion.

### Authority and PIT boundary
- Only formal official auditor/filing evidence may set formal opinion hard gates; missing pages never imply a formal conclusion.

### Mandatory verification commands
- Focused — **MANDATORY**: `python -m pytest -q tests/audit/gates`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/audit/gates` plus every previously integrated black-box seam test selected by PM manifest.
...
- [ ] 所有Frozen Spec audit cases.
- [ ] mandatory audit missing仍coverage-adjusted.
- [ ] identity/authenticity/value failure blocked.
- [ ] stars cap含0/1/no-rating不提高.
- [ ] contract供T16/T22/T23唯讀.
