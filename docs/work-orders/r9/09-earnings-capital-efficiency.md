# T09 — 獲利品質與資本效率Candidate Pillar

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 產出Pillar2候選分項、lineage與non-publishable diagnostic section。
...
- Branch: `wo/T09-earnings-capital-efficiency`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T09-earnings-capital-efficiency`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T09 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/scoring/earnings/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/scoring/earnings/
...
### Bounded output
- Contract: `EarningsCapitalEfficiencyCandidate.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/scoring/earnings/contracts/EarningsCapitalEfficiencyCandidate.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.earnings_candidate`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `metrics:record{roa:null|decimal[-1e18,1e18],operating_cash_flow:null|decimal[-1e18,1e18],accrual_ratio:null|decimal[-1e18,1e18],gross_margin:null|decimal[-1e18,1e18],asset_turnover:null|decimal[-1e18,1e18],roic:null|decimal[-1e18,1e18],free_cash_flow:null|decimal[-1e18,1e18]}, normalised_metrics:record{roa_z:null|decimal[-10,10],accrual_z:null|decimal[-10,10],margin_z:null|decimal[-10,10],turnover_z:null|decimal[-10,10],roic_z:null|decimal[-10,10],fcf_z:null|decimal[-10,10]}, cycle_flags:list<oneof[peak_margin,trough_margin,commodity_cycle,construction_lumpiness,none]>[1..8], evidence_family_ids:list<string[0..4096]>[1..32], coverage:decimal[0,1], candidate_score:null|decimal[0,100], rating_disposition:literal[NO_RATING_NOT_APPLICABLE], available_at:rfc3339_timezone_aware`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
- Insufficient comparable history yields candidate_score null with coverage reason; cycle normalization cannot invent periods.
...
- Focused — **MANDATORY**: `python -m pytest -q tests/scoring/earnings`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/scoring/earnings` plus every previously integrated black-box seam test selected by PM manifest.
...
- PM dispatch manifest is atomic and immutable: `assignment_id`, `active_binding_generation`, `eligibility_generation`, `ticket_id=T09`, `ticket_generation=R9`, `parent_sha`, `branch`, `worktree`, `owned_paths`, `forbidden_paths`, `spec_sha=36edd6b2a1b04c6282a5c30c4b4c5d89ac2535c344d6496a0d8bd54fd2009161`, `decision_map_sha=cc34f1b5f93a28b967e58be2b45f25aca6f700eba72f443dcbb3f8b1ba318b54`, `delivery_plan_sha=bd2b949ab575b01c2553269dd99d67aa385c241c924f35b43cdc1f568bd7c3e0`, `ticket_set_digest`, `lease_expires_at`, `review_deadline_at`. Missing field => no dispatch.
...
- [ ] growth/margin/earnings quality/ROIC/ROE可重算.
- [ ] F/M/Z只作適用diagnostics.
- [ ] driver/outcome不各取完整權重.
- [ ] 缺denominator null+reason.
