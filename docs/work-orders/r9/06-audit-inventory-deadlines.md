# T06 — Audit Inventory與法定申報期限重建

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 建立年度查核/季度核閱inventory，重建statutory_due_at與official_filed_at並取得會計師證據。
...
- Branch: `wo/T06-audit-inventory-deadlines`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T06-audit-inventory-deadlines`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T06 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/audit/inventory/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/audit/inventory/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/var/fixtures/audit/
...
### Bounded output
- Contract: `AuditFilingInventory.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/audit/inventory/contracts/AuditFilingInventory.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.audit_inventory`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `period:string[1..32], filing_type:oneof[annual_audit,q1_review,q2_review,q3_review], issuer_type:oneof[domestic_general,foreign_primary,foreign_secondary], industry_type:oneof[general,financial_insurance,other_regulated], fiscal_period_start:date, fiscal_period_end:date, assurance_type:oneof[audit,review], deadline_rule_id:string[1..128], deadline_rule_version:string[1..64], ordinary_due_at:rfc3339_timezone_aware, holiday_adjustment_days:uint16, approved_extension_days:uint16, statutory_due_at:rfc3339_timezone_aware, official_filed_at:null|rfc3339_timezone_aware, auditor_report_at:null|rfc3339_timezone_aware, official_filed_at_source:literal[official_filing_receipt], opinion_type:null|oneof[unmodified,qualified,adverse,disclaimer], auditor:null|string[1..256], extension_rule_id:null|string[1..128], evidence_ids:list<string[0..4096]>[0..64], coverage:decimal[0,1], rating_disposition:literal[NO_RATING_NOT_APPLICABLE]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
- Missing mandatory audit pages lowers audit coverage/confidence and forbids auditor conclusions; it does not equal adverse opinion.

### Authority and PIT boundary
- `official_filed_at` MUST NOT use auditor report date; it is sourced only from the official filing receipt/time. `statutory_due_at` is deterministically recomputed from `deadline_rule_version`, issuer/industry type, fiscal period, assurance type, ordinary due time, holiday adjustment and approved extension.
...
- Focused — **MANDATORY**: `python -m pytest -q tests/audit/inventory`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/audit/inventory` plus every previously integrated black-box seam test selected by PM manifest.
- Real-source probe — **MANDATORY**: `python -m pytest -q -m authority_probe tests/audit/inventory`; nonzero exit blocks REVIEW_READY.
...
- PM dispatch manifest is atomic and immutable: `assignment_id`, `active_binding_generation`, `eligibility_generation`, `ticket_id=T06`, `ticket_generation=R9`, `parent_sha`, `branch`, `worktree`, `owned_paths`, `forbidden_paths`, `spec_sha=36edd6b2a1b04c6282a5c30c4b4c5d89ac2535c344d6496a0d8bd54fd2009161`, `decision_map_sha=cc34f1b5f93a28b967e58be2b45f25aca6f700eba72f443dcbb3f8b1ba318b54`, `delivery_plan_sha=bd2b949ab575b01c2553269dd99d67aa385c241c924f35b43cdc1f568bd7c3e0`, `ticket_set_digest`, `lease_expires_at`, `review_deadline_at`. Missing field => no dispatch.
...
- [ ] 列assurance/report availability/auditor.
- [ ] deadline涵蓋holiday/extension.
- [ ] filed_at/report_date/due_at分離.
- [ ] 缺頁顯示mandatory gap.
- [ ] PDF hash/頁面可追溯.
