# T03 — PIT Source Admission與權威衝突 Gate

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 依四種時間、date-only午夜與authority conflict policy把source candidates admitted或fail closed。
...
- Branch: `wo/T03-pit-admission-conflicts`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T03-pit-admission-conflicts`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T03 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/pit/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/contracts/time.py
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/pit/
...
- T02 `IdentityResolution` — locator `AnalysisSnapshot.sections.identity`; producer schema `IdentityResolution.v1`; exact producer SHA must appear in dispatch bundle.
...
### Bounded output
- Contract: `AdmittedFactSet.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/pit/contracts/AdmittedFactSet.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.pit_admission`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `fact_id:string[1..128], fact_type:oneof[financial_scalar,official_close_price,valuation_multiple,corporate_action,filing_event,market_status], value:null|boolean|int64|decimal[-1e18,1e18]|string[0..4096]|date|rfc3339_timezone_aware, unit:null|string[1..32], effective_at:rfc3339_timezone_aware, announced_at:null|rfc3339_timezone_aware, available_at:rfc3339_timezone_aware, retrieved_at:rfc3339_timezone_aware, valid_from:rfc3339_timezone_aware, valid_to:null|rfc3339_timezone_aware, append_sequence:uint64, authority_rank:uint16, version_id:string[1..128], disposition:oneof[admitted,blocked_conflict,blocked_unavailable], admission_coverage:decimal[0,1], rating_disposition:literal[NO_RATING_NOT_APPLICABLE]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
- Unavailable final bar or unresolved authoritative version conflict blocks only affected facts/sections; never selects a lower authority silently.

### Authority and PIT boundary
- Admission authority is normative: effective_at/announced_at/available_at/retrieved_at remain distinct; date-only source admits next Asia/Taipei midnight; official final bar excluded before availability; unresolved authority conflict blocks affected output; append_sequence is immutable and each valid interval is half-open `[valid_from, valid_to)`.

### Mandatory verification commands
- Focused — **MANDATORY**: `python -m pytest -q tests/pit`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/pit` plus every previously integrated black-box seam test selected by PM manifest.
...
- PM dispatch manifest is atomic and immutable: `assignment_id`, `active_binding_generation`, `eligibility_generation`, `ticket_id=T03`, `ticket_generation=R9`, `parent_sha`, `branch`, `worktree`, `owned_paths`, `forbidden_paths`, `spec_sha=36edd6b2a1b04c6282a5c30c4b4c5d89ac2535c344d6496a0d8bd54fd2009161`, `decision_map_sha=cc34f1b5f93a28b967e58be2b45f25aca6f700eba72f443dcbb3f8b1ba318b54`, `delivery_plan_sha=bd2b949ab575b01c2553269dd99d67aa385c241c924f35b43cdc1f568bd7c3e0`, `ticket_set_digest`, `lease_expires_at`, `review_deadline_at`. Missing field => no dispatch.
...
- [ ] 午夜前/整點/後邊界.
- [ ] final daily bar在official available_at前不得admitted.
- [ ] authority conflict不選方便版本.
- [ ] revision append-only/半開interval.
- [ ] 同decision_time重現同admitted set.
