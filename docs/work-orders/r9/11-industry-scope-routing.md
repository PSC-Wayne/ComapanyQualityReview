# T11 — 產業Scope與Model Route

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 決定一般非金融、特殊產業與cyclical公司能否進V1。
...
- Branch: `wo/T11-industry-scope-routing`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T11-industry-scope-routing`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T11 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/industry/routing/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/industry/routing/
...
### Bounded output
- Contract: `IndustryRoute.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/industry/routing/contracts/IndustryRoute.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.industry_route`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `issuer_id:string[1..64], sector_code:string[1..32], industry_code:string[1..32], business_model_tags:list<string[1..64]>[1..32], cyclicality:oneof[defensive,moderate,cyclical,deep_cyclical], peer_rule_id:string[1..128], route_version:semver, evidence_ids:list<string[0..4096]>[1..32], route_coverage:decimal[0,1], rating_disposition:literal[NO_RATING_NOT_APPLICABLE]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
- Ambiguous industry route blocks peer-dependent outputs; issuer identity remains usable.
...
- Focused — **MANDATORY**: `python -m pytest -q tests/industry/routing`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/industry/routing` plus every previously integrated black-box seam test selected by PM manifest.
- Real-source probe — **MANDATORY**: `python -m pytest -q -m authority_probe tests/industry/routing`; nonzero exit blocks REVIEW_READY.
