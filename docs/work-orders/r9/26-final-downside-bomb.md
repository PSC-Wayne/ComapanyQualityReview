# T26 — 正式Downside哭臉、Bomb與Stress Publication Slice

**Status:** draft-for-wayne-review — implementation not authorized

**Objective:** 依G1發布三分項、普通哭臉、獨立Bomb、risk register與stress。

**Blocked by:** T07, T10, T12, T13, T14, T18, T19, T23
...
- Branch: `wo/T26-final-downside-bomb`
- Worktree: `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/.worktrees/T26-final-downside-bomb`
- Attempt: `PM_REQUIRED_AT_DISPATCH`
- Worker: T26 Worker; isolated worktree; freeze candidate SHA after handoff.
...
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/ratings/downside/
- /mnt/d/Claude_Code/Hermes/CompanyQualityResearch/tests/ratings/downside/
...
### Bounded output
- Contract: `FinalDownsideRating.v1`; schema source `/mnt/d/Claude_Code/Hermes/CompanyQualityResearch/src/company_quality/ratings/downside/contracts/FinalDownsideRating.schema.json`.
- Runtime locator: `AnalysisSnapshot.sections.final_downside`; immutable generation ID and producer candidate SHA are mandatory envelope fields.
- Required fields: `horizon_months:literal[12], applied_component_weights:record{maximum_drawdown_vulnerability:decimal[0.25,0.40],permanent_capital_loss_vulnerability:decimal[0.25,0.40],material_adverse_event_vulnerability:decimal[0.25,0.40],sum:literal[1]}, vulnerabilities:record{maximum_drawdown_vulnerability:int[1,5],permanent_capital_loss_vulnerability:int[1,5],material_adverse_event_vulnerability:int[1,5]}, crying_faces:null|int[1,5], bomb:null|record{event_id:string[1..128],event_type:oneof[formal_adverse_opinion,formal_disclaimer,confirmed_fraud,default,insolvency,major_regulatory_action,other_governed],authoritative:true,material:true,current_relevance:true,authority_source_id:string[1..128],effective_at:rfc3339_timezone_aware,expires_at:null|rfc3339_timezone_aware,evidence_ids:list<string[0..4096]>[1..32]}, diagnostic_ref:record{generation_id:uuid,locator:literal[AnalysisSnapshot.sections.downside_stress],artifact_sha256:sha256}, risk_register_ref:record{generation_id:uuid,locator:literal[AnalysisSnapshot.sections.downside_stress.risk_items],artifact_sha256:sha256}, anti_double_count_policy_version:semver, evidence_family_policy_scope:literal[quality_and_downside], evidence_family_policy_canonicalization:literal[RFC8785_JCS], evidence_family_policy_sha256:sha256, overlap_dispositions:list<record{evidence_family_id:string[1..128],primary_component:oneof[audit_reliability,earnings_capital_efficiency,cash_balance_allocation,business_moat,governance,people_adaptability,maximum_drawdown_vulnerability,permanent_capital_loss_vulnerability,material_adverse_event_vulnerability,hard_gate_only],excluded_from:list<oneof[audit_reliability,earnings_capital_efficiency,cash_balance_allocation,business_moat,governance,people_adaptability,maximum_drawdown_vulnerability,permanent_capital_loss_vulnerability,material_adverse_event_vulnerability]>[0..9],disposition:oneof[single_owner,excluded_hard_gate,excluded_duplicate],policy_rule_id:string[1..128],evidence_ids:list<string[1..128]>[1..64]}>[1..256], overlap_disposition_counts:record{single_owner:uint32,excluded_hard_gate:uint32,excluded_duplicate:uint32}, stress_summary:record{bear_equity_value_change_pct:decimal[-1e18,1e18],base_equity_value_change_pct:decimal[-1e18,1e18],bull_equity_value_change_pct:decimal[-1e18,1e18]}, coverage:decimal[0,1], confidence:decimal[0,1], no_rating_reason:null|oneof[missing_frozen_policy,insufficient_coverage,authority_conflict], policy_version:semver, freeze_decision_id:string[1..128], freeze_decision_file_sha256:sha256, evidence_ids:list<string[0..4096]>[1..256]`. Optional fields must be explicitly nullable in schema; undeclared fields are rejected by contract tests.
...
### Explicit non-goals
- Apply only frozen downside policy; do not first create risk register, raw constructs or stress pack.
...
- Frozen-policy resolution invariant: resolve T19 bytes only through the T23 frozen `evidence_family_policy_locator`; their SHA MUST equal T23 `evidence_family_policy_sha256`. `overlap_dispositions` is an exact field-preserving projection of frozen records whose primary or excluded component is one of the three downside constructs; counts equal that projection. Missing locator/SHA, changed field, extra record or count mismatch => `BLOCKED_CONTRACT`.
- Same-generation/hash invariant: `diagnostic_ref.generation_id == risk_register_ref.generation_id == FinalDownsideRating envelope.generation_id`. `diagnostic_ref.artifact_sha256` binds the canonical T18 diagnostic bytes; `risk_register_ref.artifact_sha256` MUST equal SHA-256 of the RFC8785/JCS canonical `risk_items` slice extracted from those exact T18 bytes. Any generation/hash/slice mismatch => `BLOCKED_CONTRACT`; T26 must not recompute or rewrite the risk register.
- Missing frozen downside diagnostic blocks affected face/Bomb; three faces stay separate and Bomb never overwrites model faces.
...
- Focused — **MANDATORY**: `python -m pytest -q tests/ratings/downside`.
- Cumulative — **MANDATORY**: `python -m pytest -q tests/contracts tests/ratings/downside` plus every previously integrated black-box seam test selected by PM manifest.
...
- PM dispatch manifest is atomic and immutable: `assignment_id`, `active_binding_generation`, `eligibility_generation`, `ticket_id=T26`, `ticket_generation=R9`, `parent_sha`, `branch`, `worktree`, `owned_paths`, `forbidden_paths`, `spec_sha=36edd6b2a1b04c6282a5c30c4b4c5d89ac2535c344d6496a0d8bd54fd2009161`, `decision_map_sha=cc34f1b5f93a28b967e58be2b45f25aca6f700eba72f443dcbb3f8b1ba318b54`, `delivery_plan_sha=bd2b949ab575b01c2553269dd99d67aa385c241c924f35b43cdc1f568bd7c3e0`, `ticket_set_digest`, `lease_expires_at`, `review_deadline_at`. Missing field => no dispatch.
...
- [ ] Contract tests resolve T19 policy via T23 locator/SHA, prove whitespace/key-order/escaping-equivalent semantic values produce the same RFC8785/JCS SHA, verify exact downside projection/counts, and reject unknown locator, non-JCS recipe, SHA mismatch, semantic omission/addition/rewriting and conditional-disposition violations.

- [ ] Contract tests reject unequal diagnostic/risk/output generations and reject any risk slice whose RFC8785/JCS hash is not derivable from the exact T18 diagnostic bytes.

- [ ] 三分項.
- [ ] 不足no-rating.
- [ ] Bomb authoritative/material/current.
- [ ] risk register完整.
- [ ] stress非prediction interval.
